...
class FasterWhisperASR:
    ...
    def _apply_hf_token(self):
        token = self.hf_token or os.getenv("HF_TOKEN") or os.getenv("HUGGINGFACE_HUB_TOKEN")
        if token:
            os.environ["HF_TOKEN"] = token
            os.environ["HUGGINGFACE_HUB_TOKEN"] = token
            print("[asr] HF token detected, authenticated model download enabled")
        else:
            print("[asr] warning: no HF token set, model download may be rate-limited")
